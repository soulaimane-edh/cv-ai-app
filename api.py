...
# ---------------- Config ----------------
MODEL_ID_DEFAULT = os.getenv("MODEL_ID", "gpt-5-mini")
EMB_MODEL        = os.getenv("EMB_MODEL", "sentence-transformers/all-MiniLM-L6-v2")
USE_EMBEDDINGS   = os.getenv("USE_EMBEDDINGS", "0") in ("1", "true", "True", "yes")
MAX_FILE_MB      = int(os.getenv("MAX_FILE_MB", "5"))
MAX_PAGES        = int(os.getenv("MAX_PAGES", "8"))

# ---------------- Utils fichiers/texte ----------------
...
def offline_spec_from_text(txt: str) -> dict:
    ...
    if not must and not nice:
        for k in ["python","sql","spark","airflow","aws","docker","power bi","pandas",
                  "scikit-learn","pytorch","azure","gcp","excel","dbt","tableau"]:
    ...
    return _renormalize_weights(validate_fill_spec({
        "must_have": must, "nice_to_have": nice, "experience_min_ans": exp,
        "langues": langues, "diplomes": diplomes, "certifications": certifs,
        "localisation": loc, "disponibilite_max_semaines": dispo, "poids": DEFAULT_SPEC["poids"]
    }))
...
def _chat_completion(model: str, messages: list, temperature: float = 0, max_tokens: int = 700, retries: int = 4) -> str:
    key = _get_openai_key()
    if not key or not key.startswith("sk-"):
        raise RuntimeError("OPENAI_API_KEY absente/invalide")
    ...
    raise RuntimeError(f"OpenAI indisponible après retries. {last_err}")

SPEC_SYSTEM = (
    "Tu es SPEC_BUILDER. À partir d'une fiche projet en texte libre, "
    "renvoie UNIQUEMENT un JSON valide avec le schéma: {"
    "\"must_have\":[string],\"nice_to_have\":[string],\"experience_min_ans\":number,"
    "\"langues\":{\"fr|en|...\":\"A1|A2|B1|B2|C1|C2\"},\"diplomes\":[string],"
    "\"certifications\":[string],\"localisation\":string,\"disponibilite_max_semaines\":number,"
    "\"poids\":{\"must_have\":number,\"nice_to_have\":number,\"experience\":number,"
...
def gpt_build_spec_from_text(fiche_texte: str, model_id: str = None) -> dict:
    ...
    try:
        ...
        txt = _chat_completion(model_id, [{"role":"system","content":SPEC_SYSTEM},
                                          {"role":"user","content":fiche_texte}], 0, 700).strip()
...
# ---------------- Scoring (must/nice + règles) ----------------
def _score_keywords(cv_text: str, spec: Dict[str,Any]) -> Tuple[float, Dict[str, Any]]:
    LOW = cv_text.lower()
...
def _score_embeddings(cv_text: str, spec: Dict[str,Any],
                      thr_must: float = 0.28, thr_nice: float = 0.25) -> Tuple[float, Dict[str, Any]]:
    # tente embeddings, fallback keyword si indisponible
    try:
        ...
        sims_must=[]; sims_nice=[]
        for sk in must: sims_must.append(max(0.0, sim(sk)))
        for sk in nice: sims_nice.append(max(0.0, sim(sk)))
        pts = 0.0
        if must:
            avg = mean(sims_must); cover = mean([1.0 if s>=thr_must else 0.0 for s in sims_must])
            pts += w_must * min(1.0, max(0.0, avg)) * cover
        if nice:
            avg = mean(sims_nice); cover = mean([1.0 if s>=thr_nice else 0.0 for s in sims_nice])
            pts += w_nice * min(1.0, max(0.0, avg)) * cover
        return round(pts,2), {"must": list(zip(must, sims_must)), "nice": list(zip(nice, sims_nice))}
    except Exception:
        return _score_keywords(cv_text, spec)
...
def score_autres(extraction: Dict[str,Any], spec: Dict[str,Any]) -> float:
    P = spec.get("poids", {})
    w_exp = float(P.get("experience",15)); w_lang = float(P.get("langues",10))
    w_dc  = float(P.get("diplomes_certifs",10)); w_locd = float(P.get("localisation_dispo",5))
    pts = 0.0
    ...
    if lang_req:
        per = w_lang / max(1,len(lang_req))
        for code, req in lang_req.items():
            have = next((d.get("niveau") for d in extraction.get("langues",[])
                         if (d.get("code") or "").lower()==code.lower()), None)
            if have and ORDER.get((have or "").upper(),0) >= ORDER.get((req or "").upper(),0):
                pts += per
    if extraction.get("diplomes_obtenus") or extraction.get("certifications"):
        pts += w_dc
...
class InAnalyze(BaseModel):
    spec_text: str = ""     # texte de la fiche projet (le plugin lui envoie le contenu extrait du fichier)
    cvs: List[CVItem] = []  # CVs encodés en base64
    lang: str = "fr"

@app.get("/healthz")
def health():
    return {"ok": True, "use_embeddings": bool(USE_EMBEDDINGS)}

@app.post("/analyze_cv")
def analyze_cv(inp: InAnalyze):
    # 1) Construire la spec (LLM si clé dispo, sinon offline)
    spec = gpt_build_spec_from_text(inp.spec_text or "", model_id=MODEL_ID_DEFAULT)

    results = []
    for it in inp.cvs:
        raw = base64.b64decode(it.content_b64)
        if len(raw) > MAX_FILE_MB * 1024 * 1024:
            results.append({"file": it.name, "error": f"Fichier > {MAX_FILE_MB} Mo"})
            continue

        cv_text = read_text_from_named_bytes(it.name, raw)
        extraction = offline_extract(cv_text)  # simple, robuste; tu peux brancher LLM si voulu

        # 2) Points compétences
        if USE_EMBEDDINGS:
            pts_mn, _ = _score_embeddings(cv_text, spec)
        else:
            pts_mn, _ = _score_keywords(cv_text, spec)

        # 3) Règles strictes
        pts_autres = score_autres(extraction, spec)

        score_final = float(min(100.0, round(pts_mn + pts_autres, 2)))
        results.append({"file": it.name, "score_final": score_final})
