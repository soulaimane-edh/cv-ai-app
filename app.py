...
def build_commentaire_deterministe(score_final, evidences, extraction, spec):
    ...
    def decision_band(score):
        return (
            "Très bon match – à convoquer"
            if score >= 85
            else "Bon match – à prioriser"
            if score >= 70
            else "Moyen – à examiner"
            if score >= 55
            else "Faible – non prioritaire"
        )
